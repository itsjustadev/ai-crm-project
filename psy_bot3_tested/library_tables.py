...
def add_has_analysis(chat_id, session):
...
def add_in_deal_to_close(chat_id, stage_close, session):
...
def get_count_to_close(chat_id: int, session):
...
def delete_count_to_close(chat_id: int, session):
    user = session.query(DealsToClose).filter_by(chat_id=chat_id).first()
    if user:
        session.delete(user)
        session.commit()
...
def check_has_analysis(chat_id: int, session):
...
def add_new_message(chat_id, new_message, username, name, media_link, file_name, session):
...
def add_shorted_history(chat_id, shorted_history, shorted_rows: int, session):
    try:
...
def add_message_from_client(chat_id, message, session):
    try:
        existing_entry = session.query(
...
def delete_shorted_history(chat_id: int, session):
    try:
        existing_entry = session.query(
            ShortedHistory).filter_by(chat_id=chat_id).first()
        if existing_entry:
...
def delete_messages_from_client(chat_id: int, session):
    try:
        existing_entry = session.query(
            MessagesFromClient).filter_by(chat_id=chat_id).first()
        if existing_entry:
...
def merge_shorted_history(chat_id, shorted_history, shorted_rows: int, session):
    try:
        existing_entry = session.query(
            ShortedHistory).filter_by(chat_id=chat_id).first()
        if existing_entry:
...
def check_username_exists(username, session):
    existing_user = session.query(
        UserNames).filter_by(username=username).first()
    return existing_user is not None


def check_new_prompt_exists(chat_id, session):
    existing_user = session.query(
        NewPrompt).filter_by(chat_id=chat_id).first()
    return existing_user is not None


def check_new_stage_exists(session):
    existing_user = session.query(
        AmoNewStages).first()
    return existing_user is not None


def check_shorted_history_exists(chat_id, session):
    existing_short_history = session.query(
        ShortedHistory).filter_by(chat_id=chat_id).first()
    return existing_short_history is not None


def check_name_exists(name, session):
    existing_user = session.query(UserNames).filter_by(name=name).first()
    return existing_user is not None


def add_new_username_and_name(chat_id, session):
    username = generate_random_string(12)
    while check_username_exists(username, session):
        username = generate_random_string(12)
    name = 'Клиент' + generate_random_digits(8)
    while check_name_exists(name, session):
        name = 'Клиент' + generate_random_digits(8)
...
def add_new_username(chat_id, session):
    username = generate_random_string(12)
    while check_username_exists(username, session):
        username = generate_random_string(12)
...
def add_new_name(chat_id, session):
    name = 'Клиент' + generate_random_digits(8)
    while check_name_exists(name, session):
        name = 'Клиент' + generate_random_digits(8)
...
def add_new_amo_stage(chat_id, username, name, status_id, session):
...
def add_new_prompt(chat_id, prompt, session):
    new_addition = NewPrompt(chat_id=chat_id, prompt=prompt)
    session.merge(new_addition)
    session.commit()


def add_recent_message(chat_id, stage_in_amo, session):
...
def add_recent_message_another_time(chat_id, stage_in_amo, time_hours, session):
    result = session.query(RecentMessages).filter_by(chat_id=chat_id).first()
    if result:
        if result.stage_in_amo not in ['142', '143']:
            new_addition = RecentMessages(
...
def delete_recent_message(chat_id, session):
    result = session.query(RecentMessages).filter_by(chat_id=chat_id).first()
    if result:
...
def add_new_lead_id(chat_id, lead_id, session):
    new_addition = LeadId(chat_id=chat_id, lead_id=lead_id)
    session.merge(new_addition)
    session.commit()


def add_in_users(user_name, name, chat_id, user_id, session):
...
def get_username_by_chat(chat_id, session):
...
def get_lead_id_by_chat(chat_id: int, session):
...
def get_first_in_amo_stage(session):
    list_new = []
...
def get_chat_by_user_id(user_id, session):
...
def get_new_prompt(chat_id, session):
...
def get_username_by_user_id(user_id, session):
    user = session.query(Users).filter_by(user_id=user_id).first()
    if user:
        return user.username
    else:
...
def get_name_by_user_id(user_id, session):
    user = session.query(Users).filter_by(user_id=user_id).first()
    if user:
        return user.name
    else:
...
def get_messages_from_client(chat_id: int, session):
...
def get_shorted_history(chat_id, session):
...
def get_shorted_rows_history(chat_id, session):
...
def get_name_by_chat(chat_id: int, session):
    user = session.query(UserNames).filter_by(chat_id=chat_id).first()
    if user:
        return user.name
    else:
...
def delete_first_message(session):
    try:
        first_record = session.query(ListOfNewMessages).first()
        if first_record:
...
def delete_amo_new_stage(session):
    try:
...
def print_all_messages(session):
...
def delete_all_lead_id(session):
    all_records = session.query(LeadId).all()
    if all_records:
        for item in all_records:
            session.delete(item)
            session.commit()
...
def delete_new_prompt(chat_id, session):
...
def update_time_recent_message(chat_id, session):
...
def check_recent_messages(minutes: int, session):
...
def delete_all_recent_messages(session):
    try:
...
def set_busy_bot(chat: int, session):
...
def set_free_bot(chat: int, session):
    activity = session.query(IsBotFree).filter_by(chat_id=chat).first()
    try:
...
def update_false(chat: str, session):
    try:
        activity = session.query(BotActivity).filter_by(chat_id=chat).first()
