    ...
    while True:
        ...
        if BotActivity.check_new_stage_exists():
            ...
            try:
                ...
                if not BotActivity.is_bot_active(str(chat)):
                    logger_controling_deals.info(
                        f'bot is not active to "handle_change_stage" for {chat} '+str(datetime.datetime.now()), exc_info=True)
                elif status_id == STAGE_IN_AMO_4:
                ...
                elif status_id not in [STAGE_IN_AMO_1, STAGE_FOR_MANAGER, STAGE_FOR_SALE, STAGE_FOR_CLOSED_DEALS, STAGE_FOR_DONE_DEALS]:
                    if status_id == STAGE_IN_AMO_3:
        ...
        if BotActivity.has_first_message():
            ...
            try:
                ...
                if amo_message in ['start']:
                    if BotActivity.is_bot_active(str(chat)):
                        ...
                        logger_controling_deals.info(
                            f'bot is not active to "Start-from-amo" for {chat} '+str(datetime.datetime.now()), exc_info=True)
                ...
                if amo_message in ['resume']:
                    if BotActivity.is_bot_active(str(chat)):
                        ...
                        logger_controling_deals.info(
                            f'bot is not active to "Resume-from-amo" for {chat} '+str(datetime.datetime.now()), exc_info=True)
                    list_for_new_message.clear()
                    BotActivity.delete_first_message()
                    continue
                if amo_message in ['resume-hard']:
                    if BotActivity.check_existing(str(chat)):
                        BotActivity.update_true(str(chat))
                    enable_project0(str(chat))
                    await function_for_initializing_conversation(chat, user_name, name, bot)
        ...
        try:
            ...
            if 'a0c4' in client_id:
                async with httpx.AsyncClient() as client:
                    response = await client.post(f'http://localhost:8080/input_handler_psy_bot/{text}', json=data.dict())
...
async def get_group_analysis(lead_id, user_name, bot):
    try:
        ...
        if PART_OF_ANALYSIS in str(analysis_result):
            ...
            logger_controling_deals.error(f'NO ANALYSIS FOR {lead_id}')
    except Exception as e:
        chat_gpt_response = ''
        await bot.send_message(CHAT_FOR_LOGS, f'Необходим анализ для {user_name} сделки {lead_id}')
        logger_controling_deals.error(
            f'NO ANALYSIS FOR {lead_id} because of '+str(e), exc_info=True)
...
async def helper_for_psy_handle_amo_message(data: IncomingMessage, request: Request):
    ...
    if 'localhost' in headers['host']:
        try:
            ...
            username = client_id.split("-")[-1]
            if not name or not username or name == 'None' or username == 'None':
                username = psy_get_username(chat_id)
    ...
    if 'amoCRM' in headers['user-agent']:
        try:
            ...
            if email == 'example-psy.client@example.com':
                amo_pipeline_change(lead_id, 7557006, 62598898)
        except Exception as e:
            logging.error(str(e), exc_info=True)
