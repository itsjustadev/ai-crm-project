        ...
        if activity:
            for item in activity:
                ...
                if time_since_last_message >= timedelta(minutes=minutes):
                    list_of_not_recent.append((item.chat_id, item.stage_in_amo))
        else:
            print('no chat id in table for update_true function0')
    except Exception as e:
        print(str(e))
    finally:
...
def free_all_bot():
    ...
    if activity:
        ...
        print('no chat id in table for update_true function1')
